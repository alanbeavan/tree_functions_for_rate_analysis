...
def get_args():
    ...
    if len(sys.argv) == 6:
        ...
        print("\nUSAGE: python get_branch_lengths_by_node.py treedir outgroups treeslist species_tree outfile\n")
        exit()

def extract_node(treefile, treedir, outgroups):
    """Find the node to analyse in ete3."""
    index = int(treefile.split(".")[0].split("_")[1])
    outgroup = outgroups[index]
